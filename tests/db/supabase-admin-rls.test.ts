...
describe("Supabase admin RLS contract", () => {
  ...
  it.each([
    ["insert", /create policy "Admins can insert products"[\s\S]*?to authenticated[\s\S]*?with check \(public\.is_admin\(\)\);/],
    ["update", /create policy "Admins can update products"[\s\S]*?to authenticated[\s\S]*?using \(public\.is_admin\(\)\)[\s\S]*?with check \(public\.is_admin\(\)\);/],
    ["delete", /create policy "Admins can delete products"[\s\S]*?to authenticated[\s\S]*?using \(public\.is_admin\(\)\);/]
  ])("requires server-side admin authority for product %s", (_operation, policy) => {
    expect(schema).toMatch(policy);
  });

  it.each([
    ["upload", /create policy "Admins can upload product images"[\s\S]*?to authenticated[\s\S]*?with check \(bucket_id = 'products' and public\.is_admin\(\)\);/],
    ["update", /create policy "Admins can update product images"[\s\S]*?to authenticated[\s\S]*?using \(bucket_id = 'products' and public\.is_admin\(\)\)[\s\S]*?with check \(bucket_id = 'products' and public\.is_admin\(\)\);/],
    ["delete", /create policy "Admins can delete product images"[\s\S]*?to authenticated[\s\S]*?using \(bucket_id = 'products' and public\.is_admin\(\)\);/]
  ])("requires server-side admin authority for product image %s", (_operation, policy) => {
    expect(schema).toMatch(policy);
  });

  it("drops legacy authenticated-wide policies before creating admin-only replacements", () => {
